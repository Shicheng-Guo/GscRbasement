...
beginning <- as.Date(Sys.Date()-28)
date<-Sys.Date()

#setwd("/home/sguo/Dropbox/stock/mystock")
setwd("C:\\Users\\guosa\\Documents\\stock")
input<-read.table("ETF.db",sep="\t")
rlt<-c()
Return<-c()
for(i in 1:nrow(input)){
  ...
  Max=as.numeric(Object[1,4])
  Now=as.numeric(tail(Object,1)[,4])
  Mid<-as.numeric(Object[round(nrow(Object)/2),4])
  return=-sign(Mid-Max)*(Now/Max)
  Return<-c(Return,return)
  print(c(i,symbol,nrow(Object),return))
  
  #file=paste(symbol,"png",sep=".")
  #png(file,width = 8, height = 8, units = 'in', res = 800)
  #chartSeries(Object,subset='last 10 months',main=symbol,TA=c(addVo(),addMACD(),addBBands(),addCCI(),addRSI(n = 12, wilder = TRUE),addEMA(n=60, col="blue"),addEMA(n=30, col="yellow")))
  #dev.off()
}
Return
names(Return)<-input[,1]  
data.frame(Return)
write.table(Return,file="rully.txt",quote=F,sep="\t",row.names = T,col.names = F)
