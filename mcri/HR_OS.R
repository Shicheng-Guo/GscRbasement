...
i=grep(Symbol2ENSG(c("MTMR8"))[1,2],rownames(input))
i=grep(Symbol2ENSG(c("MTMR9"))[1,2],rownames(input))
i=grep(Symbol2ENSG(c("KPNA4"))[1,2],rownames(input))
...
for(i in 1:nrow(input)){
  ...
  write.table(HR,file="TCGA-HR-OS-Meta-Pvalue-MTMR8.txt",sep="\t",col.names = NA,row.names = T,quote=F)
