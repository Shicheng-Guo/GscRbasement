...
brca<-read.table("https://raw.githubusercontent.com/Shicheng-Guo/HowtoBook/master/TCGA/brca/meth450/TCGA-BRCA-BUR-PAN-TopVar.txt")
newinput<-input[,c(1,match(rownames(brca),colnames(input)))]
for(i in 2:ncol(newinput)){
  pdf(paste(colnames(newinput)[i],i,"beta.pdf",sep="."))
  boxplot(newinput[,i]~newinput[,1])
  dev.off()
  print(i)
}
...
newinput<-input[,c(1,match(names(P[head(order(P),n=4000)]),colnames(input)))]
...
load("~/hpc/methylation/TCGA-BRCA-RNAseq-FPKM-UQ.Symbol.DEG.RData")
DMG<-rlt
AOS<-read.table("~/hpc/methylation/TCGA_BRCA_FPKM-UQ.DGE_OS_HR_PanDiff.All.txt",head=T,sep="\t")
SOS<-read.table("~/hpc/methylation/TCGA_BRCA_FPKM-UQ.DGE_OS_HR_PanDiff.All.txt",head=T,sep="\t")
bedR<-read.table("https://raw.githubusercontent.com/Shicheng-Guo/HowtoBook/master/TCGA/brca/Wu/Initial_328_region.bed",head=T)
ProteinAtlas<-read.table("https://raw.githubusercontent.com/Shicheng-Guo/ProteinAtlas/master/Pathology.OS.tsv",sep="\t",head=T)
ProteinAtlasBRCA<-subset(ProteinAtlas,Cancer=="breast cancer" & prognostic...favourable<0.01)
OO1<-data.frame(bedR,DMG[match(bedR$Symbol,rownames(DMG)),])
OO2<-subset(OO1,Estimate<0 & Pr...t..<10^-5)
OO3<-data.frame(OO2,ProteinAtlasBRCA[match(rownames(OO2),ProteinAtlasBRCA$Gene.name),])
OO4<-subset(OO3,prognostic...favourable<0.01)
OO3<-data.frame(OO2,SOS[match(rownames(OO2),SOS$Symbol),])
OO4<-subset(OO3,Estimate.1<0 & Pr...t...1<10^-20)
dim(OO4)
BEDR<-c()
for(i in unique(OO4$Symbol)){
  xx<-which(bedR$Symbol %in% i)
  BEDR<-rbind(BEDR,bedR[xx[which.min(bedR[xx,5])],])
}
write.table(BEDR,file="Wu_Target.txt",sep="\t",col.names = NA,row.names = T,quote=F)
...
OO3<-data.frame(OO2,ProteinAtlas[match(OO2$Symbol,ProteinAtlas$Gene.name),])
OO4<-subset(OO3,prognostic...favourable<0.01)
