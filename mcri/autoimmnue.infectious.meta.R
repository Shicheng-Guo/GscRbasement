...
for(i in 1:length(RAG)){
  gene<-RAG[i]
  my_query <- paste("(SNP[TIAB] or polymorphism[TIAB]) AND association[TIAB] AND (autoimmune[TIAB] or infect[TIAB]) AND",gene,"[TIAB]")
...
write.table(Count,file="autoimmnue.infectious.meta.txt",sep="\t",quote=F)
getwd()
...
for(i in 1:length(RAG)){
  gene<-RAG[i]
  my_query <- paste("methylation[TIAB] AND Cholangiocarcinoma[TIAB] AND",gene,"[TIAB]")
  my_query <- get_pubmed_ids(pubmed_query_string = my_query)
  Count<-rbind(Count,c(as.character(RAG[i]),my_query$Count,paste(unlist(my_query$IdList),collapse=";")))
  print(c(as.character(RAG[i]),my_query$Count))
}
write.table(Count,file="Cholangiocarcinoma.methylation.meta.txt",sep="\t",quote=F)
...
for(i in 1:length(RAG)){
  gene<-RAG[i]
  my_query <- paste("methylation[TIAB] AND meta[TI] AND",gene,"[TIAB]")
  my_query <- get_pubmed_ids(pubmed_query_string = my_query)
  Count<-rbind(Count,c(as.character(RAG[i]),my_query$Count,paste(unlist(my_query$IdList),collapse=";")))
  print(c(as.character(RAG[i]),my_query$Count))
}
