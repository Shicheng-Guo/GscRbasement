...
for(i in 1:nrow(input)){
  ...
  before<-max(na.omit(head(tail(Object,30),15)[,1]))
  after<-max(na.omit(head(tail(Object,15),15)[,1]))
  max.day<-which.max(head(tail(Object,15),15)[,1])
  v.before<-mean(na.omit(head(tail(Object,30),15)[,5]))
  v.after<-Object[max.day,5]
  if(after/before>2 && v.after>5*v.before){
  print(c(i,symbol))
...
write.table(rlt,file,quote=F,sep="\t",row.names = F,col.names = F)
