...
dge<-read.csv("https://raw.githubusercontent.com/Shicheng-Guo/cholangiocarcinoma/master/chol.meta.table.pvalue.csv")
...
rlt<-merge(dge,os,by="X")
rlt$pick<-log(rlt$TE.fixed,2)*rlt$beta
subset(rlt,pval<0.05 & pval.fixed<0.05 & pick>0)
subset(rlt,pval<0.05 & pval.fixed<0.05 & beta>0 & TE.fixed>1)
subset(rlt,pval<0.05 & pval.fixed<0.05 & beta<0 & TE.fixed<1)
write.csv(rlt,file="chol.pancancer.DGE.OS.csv",quote=F)
